...
const CatalogItemHorizontal: React.FC<CatalogItemProps> = ({ id, title, description, image, tags, typeOfUnit, discounts, weight, points, price, rating }) => {
    ...
    const addItemsToCart = () => {
        items.map(({ weight, price, count }, idx) => {
            const itemPoints = points[idx]
            if (count !== 0) {
                dispatch(
                    addToCart(
    ...
    return (
        <div className="item-horizontal">
            <div className="item-tags">
                ...
                {(tags.includes("discount") || tags.includes("bonus")) && <div className="item-tag-stock">{`Акция ${discounts.find((el) => el !== 0)}%`}</div>}
            ...
            <div className="item-horizontal-information">
                <div className="item-horizontal-information-top">
                    <div className="item-horizontal-information-top-left">
                        <h1 className="item-horizontal-information-top-left-title">{title}</h1>
                        <span className="item-horizontal-information-top-left-description">{description}</span>
                    </div>
                ...
                <div className="item-horizontal-information-bottom">
                    ...
                    <span>Бонус</span>
                    <span className="orangeText">Цена</span>
                    {items.map(({ weight, count, points, totalPrice, discounts }, idx) =>
                        <React.Fragment key={idx}>
                            <span>{weight}{typeOfUnit}.</span>
                            ...
                            <span className='redText'>{renderSVG(shareType)}</span>
                            <span className="item-big-about-chapter-bonus">
                                {points}
                                ...
                                discounts !== 0 && discounts ?
                                    totalPrice !== 0 ?
                                        <div className="item-big-about-chapter-block">
                                            ...
                                            <span className="item-big-about-chapter-block-lastPrice">{price[idx] * count}₽</span>
                                            <span className="item-big-about-chapter-price">{items[idx].price * count}₽</span>
                                        </div>
                                        : <span className="item-big-about-chapter-price">{Math.round(items[idx].price - items[idx].price / 100 * discounts) * count}₽</span>
                                    :
                                    <span className='item-big-about-chapter-price'>{items[idx].price * count}₽</span>
                            }
                        </React.Fragment>
                ...
                <div className="item-horizontal-information-total">
                    <div className="item-horizontal-information-total-start">
                ...
                <div className="item-horizontal-information-total">
                    <span className="item-horizontal-information-total-reviews">Отзывы: 12 </span>
...
export default CatalogItemHorizontal;
