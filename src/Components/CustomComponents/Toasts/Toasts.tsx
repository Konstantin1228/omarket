    ...
    <div className="toasts" >
      {
        toasts.map(({ title, id, typeOfUnit, weight, img, count, message, isComplete }) => (
          title && typeOfUnit && weight && count ?
            <ToastItem key={id} id={id} title={title} count={count} img={img || "https://i.ibb.co/dkm3qTZ/no-image.png"} typeOfUnit={typeOfUnit} weight={weight} />
