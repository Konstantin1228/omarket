...
interface CartRightType {
    isValid?: boolean
}
const CartRight: React.FC<CartRightType> = (isValid) => {
    ...
    return (
        <div className="cart__inner-notEmpty-makeOrder">
            ...
            <button type='submit' className={isValid ? `cart__inner-notEmpty-makeOrder-submitOrder` : `cart__inner-notEmpty-makeOrder-submitOrder-false`} onClick={() => setStage(stage + 1)} >
                Оформить заказ
            </button>
        </div>
    )
